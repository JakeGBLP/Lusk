...
public class EvtBeacon extends SkriptEvent {
    ...
    static {
        if (Skript.classExists("io.papermc.paper.event.block.BeaconActivatedEvent") && Skript.classExists("io.papermc.paper.event.block.BeaconDeactivatedEvent")) {
            Skript.registerEvent("Beacon on/off", EvtBeacon.class, CollectionUtils.array(BeaconActivatedEvent.class, BeaconDeactivatedEvent.class), "beacon [:de]activat(e[d]|ing)")
    ...
    public boolean init(Literal<?> @NotNull [] args, int matchedPattern, @NotNull ParseResult parseResult) {
        activating = parseResult.hasTag("de");
        return true;
    }
