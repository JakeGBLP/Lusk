...
@Examples({""})
@Since("1.0.2")
    public class ExprDisplayViewRange extends SimpleExpression<Float> {
    static {
        Skript.registerExpression(ExprDisplayViewRange.class, Float.class, ExpressionType.COMBINED,
                "[the] [display] view range of %entity%");
    }
    ...
    protected Float @NotNull [] get(@NotNull Event e) {
        ...
        if (entity != null) {
            if (entity.getType() == EntityType.TEXT_DISPLAY || entity.getType() == EntityType.BLOCK_DISPLAY || entity.getType() == EntityType.ITEM_DISPLAY) {
                return new Float[]{((Display) entity).getViewRange()};
            }
        }
        return new Float[0];
    }
    ...
    public void change(@NotNull Event e, Object @NotNull [] delta, Changer.@NotNull ChangeMode mode) {
        ...
        float value = display.getViewRange();
        switch (mode) {
            case RESET, DELETE:
                value = 0;
                break;
    ...
    public @NotNull String toString(@Nullable Event e, boolean debug) {
        if (e == null) return "";
        return "the view range of " + entityExpression.getSingle(e);
    }
}