...
public class ExprDecimalRound extends SimpleExpression<Number> {
    static {
        Skript.registerExpression(ExprDecimalRound.class, Number.class, ExpressionType.SIMPLE,
                "%number% rounded down[ward[s]] to %number% decimal places",
                        "%number% rounded to %number% decimal places",
                        "%number% rounded up[ward[s]] to %number% decimal places");
    }
    ...
    public @NotNull String toString(@Nullable Event e, boolean debug) {
        return number + " rounded " + (down == -1 ? "down " : (down == 1 ? "up " : "")) + "to " + index + " decimal places" ;
    }
}
