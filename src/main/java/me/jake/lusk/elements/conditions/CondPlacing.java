...
@Examples({"""
                            on potting of cornflower:
                              if the plant is being placed:
                                ...
                                """})
@Since("1.0.0")
public class CondPlacing extends Condition {

    static {
        if (Skript.classExists("io.papermc.paper.event.player.PlayerFlowerPotManipulateEvent")) {
            Skript.registerCondition(CondPlacing.class, "[the] ((flower|plant) [item]|item) is[negated:( not|n't)] being (placed:placed|picked up)");
        }
    }
    ...
    public @NotNull String toString(@Nullable Event event, boolean debug) {
        return "Flower Pot Manipulate Event is Placing";
    }
