...
public class CondTriggerAdvancements extends Condition {
    ...
    static {
        if (Skript.classExists("io.papermc.paper.event.player.PlayerInventorySlotChangeEvent")) {
            Skript.registerCondition(CondTriggerAdvancements.class, "[the] event (should|will) [negated:not] trigger [any] advancements",
                                                                       "[the] event (shouldn't|won't) trigger [any] advancements");
        }
    }
    ...
    public @NotNull String toString(@Nullable Event event, boolean debug) {
        return "Slot Change event should trigger advancements";
    }
