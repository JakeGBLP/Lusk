...
import static it.jakegblp.lusk.version.nms.v1_21_4.NMS_1_21_4.NMS;

public class NMSPlayer_1_21_4 extends NMSPlayer<ServerPlayer> {

    private final Entry entry;

    public NMSPlayer_1_21_4(ServerPlayer player, int id) {
        super(player, id);
        entry = new Entry(getServerPlayer().getUUID(), getServerPlayer().getGameProfile(), true, 0,
                GameType.CREATIVE, getServerPlayer().getDisplayName(), true, 0, null);
    }
    ...
    public void update(ServerPlayer serverPlayer) {
        NMS.sendPacket(new ClientboundPlayerInfoUpdatePacket(EnumSet.of(ClientboundPlayerInfoUpdatePacket.Action.ADD_PLAYER,
                ClientboundPlayerInfoUpdatePacket.Action.UPDATE_LISTED), List.of(serverPlayer)), serverPlayer);
        //ChunkMap.TrackedEntity trackedEntity = nmsPlayer.serverLevel().getChunkSource().chunkMap.entityMap.get(nmsPlayer.getId());
        NMS.sendEntitySpawnPacket(serverPlayer, 0, serverPlayer.position(), serverPlayer.getBukkitEntity());
    }
