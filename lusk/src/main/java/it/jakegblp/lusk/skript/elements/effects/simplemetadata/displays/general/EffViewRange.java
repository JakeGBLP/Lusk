...
@Name("[NMS] Player - Fake Display View Range")
@Description("""
        Sets display view range of a real or fake entity per player
        """)
@Examples({"""
        fake display with id 12 view range to 20 for all players
        """
})
@Keywords({
...
@Since("2.0.0")
public class EffViewRange extends Effect {
    ...
    protected void execute(Event event) {
        AddonUtils.sendEasyMetadata(playerExpression.getArray(event), new EntityMetadata(Map.of(MetadataKeys.DisplayKeys.VIEW_RANGE, numberExpression.getSingle(event).floatValue())), entityOrId.getArray(event));
    }

    @Override
    public String toString(@Nullable Event event, boolean b) {
        return "client display view range";
    }
