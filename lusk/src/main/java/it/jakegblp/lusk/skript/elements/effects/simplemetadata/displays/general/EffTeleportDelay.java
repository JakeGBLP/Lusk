...
@Name("[NMS] Player - Fake Display Teleport Duration")
@Description("""
        Sets display teleportation duration of a real or fake entity per player
        """)
@Examples({"""
        fake display with id 12 teleportation duration to 20 for all players
        """
})
@Keywords({
...
@Since("2.0.0")
public class EffTeleportDelay extends Effect {
    ...
    protected void execute(Event event) {
        AddonUtils.sendEasyMetadata(playerExpression.getArray(event), new EntityMetadata(Map.of(MetadataKeys.DisplayKeys.TELEPORT_INTERPOLATION_DURATION, numberExpression.getSingle(event).intValue())), entityOrId.getArray(event));
    }

    @Override
    public String toString(@Nullable Event event, boolean b) {
        return "client display teleportation duration";
    }
