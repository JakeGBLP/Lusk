...
@Name("[NMS] Player - Fake Display Shadow Radius")
@Description("""
        Sets display shadow radius of a real or fake entity per player
        """)
@Examples({"""
        fake display with id 12 shadow radius to 20 for all players
        """
})
@Keywords({
...
@Since("2.0.0")
public class EffShadowRadius extends Effect {
    ...
    protected void execute(Event event) {
        AddonUtils.sendEasyMetadata(playerExpression.getArray(event), new EntityMetadata(Map.of(MetadataKeys.DisplayKeys.SHADOW_RADIUS, numberExpression.getSingle(event).floatValue())), entityOrId.getArray(event));
    }

    @Override
    public String toString(@Nullable Event event, boolean b) {
        return "client display shadow radius";
    }
