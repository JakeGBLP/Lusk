...
import it.jakegblp.lusk.skript.modules.packets.expressions.constructors.ExprSecPlayerPositionPacket;
...
public class ExprPositionPacketComponents extends PropertyExpression<PlayerPositionPacket, Object> {
    ...
    public static void register(SyntaxRegistry syntaxRegistry) {
        AddonUtils.registerPropertyExpression(syntaxRegistry, ExprPositionPacketComponents.class, ExprPositionPacketComponents::new, Object.class,
                ExprSecPlayerPositionPacket.revamped ? "(:relative|absolute) [packet] (position:location|:position|:velocity|(:x|:y|:z) [:velocity]|(:yaw|:pitch))"
                        : "(:relative|absolute) [packet] (position:location|:position|:x|:y|:z|(:yaw|:pitch))", "playerpositionpackets");
    }
    ...
    protected Object[] get(Event event, PlayerPositionPacket[] source) {
        return CommonUtils.map(source, packet -> {
            ...
            if (relative == currentFlags.contains(relativeFlag)) {
                return switch (relativeFlag) {
                    case X -> packet.getPositionX();
                    case Y -> packet.getPositionY();
                    case Z -> packet.getPositionZ();
    ...
    public void change(Event event, Object @Nullable [] delta, Changer.ChangeMode mode) {
        ...
        for (PlayerPositionPacket packet : getExpr().getAll(event)) {
            ...
            if (position) {
                ...
                switch (relativeFlag) {
                    case X -> packet.setPositionX((Double) finalValue);
                    case Y -> packet.setPositionY((Double) finalValue);
                    case Z -> packet.setPositionZ((Double) finalValue);
