...
public class TexturesPayload {
    ...
    public static @NotNull TexturesPayload fromJson(@NotNull String json, @Nullable String signature) {
        ...
        var timestampElement = root.get("timestamp");
        if (timestampElement != null && !timestampElement.isJsonNull())
            payload.timestamp = timestampElement.getAsLong();

        var profileIdElement = root.get("profileId");
        if (profileIdElement != null && !profileIdElement.isJsonNull())
            payload.profileId = NullabilityUtils.convertIfNotNull(profileIdElement.getAsString(), CommonUtils::fromCompactUUID);

        var profileNameElement = root.get("profileName");
        if (profileNameElement != null && !profileNameElement.isJsonNull())
            payload.profileName = profileNameElement.getAsString();

        var signatureRequiredElement = root.get("signatureRequired");
        if (signatureRequiredElement != null && !signatureRequiredElement.isJsonNull())
            payload.signatureRequired = signatureRequiredElement.getAsBoolean();
    ...
    public static @Nullable TexturesPayload fromProfile(@NotNull PlayerProfile playerProfile) {
